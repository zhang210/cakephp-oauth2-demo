...
<a class="btn btn-primary" href="<?php echo 'http://' . $_SERVER['HTTP_HOST'] ?>/server/authorize?response_type=token&client_id=<?php echo $config['client_id']; ?>&redirect_uri=<?php echo 'http://' . $_SERVER['HTTP_HOST'] ?>/client/receive_implicit_token&state={{ session_id }}">Authorize</a>
